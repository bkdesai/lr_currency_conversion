...
function addManifest(zip) {
    ...
    manifest.webContextPath = project_1.default.jar.webContextPath;
    manifest.addProvideCapability('osgi.webresource', `osgi.webresource=${pkgJson.name};version:Version="${bundleVersion}"`);
    if (project_1.default.l10n.supported) {
        ...
        manifest.addProvideCapability('liferay.resource.bundle', `resource.bundle.base.name="content.${bundleName}"`);
    }
    ...
    Object.entries(project_1.default.jar.customManifestHeaders).forEach(([key, value]) => manifest.addCustomHeader(key, value));
...
function getCompressionConfiguration() {
    if (project_1.default.jar.compressionLevel === 0) {
        return { compression: 'STORE' };
    }
    else {
        return {
            compression: 'DEFLATE',
            compressionOptions: { level: project_1.default.jar.compressionLevel },
        };
    }
}
/**
